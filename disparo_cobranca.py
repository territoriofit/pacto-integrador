...
def acompanhamento_ativo(akey: str, sb: dict, lead_id: str, valor: str) -> bool:
    ...
    try:
        ...
        transcript = "\n".join(
            ...
            for m in hist if (m.get("content") or "").replace("﻿", "").strip())
        if not transcript:
            return False
        import anthropic
...
def _envia_relatorio_whats(linhas: list[str]) -> None:
    ...
    destino = os.environ.get("RELATORIO_PARA", "5516992290338").replace("﻿", "").strip()
    if not token:
...
def main() -> int:
    ...
    direta_clientes = [c.replace("﻿", "").strip() for c in
