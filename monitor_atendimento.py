...
def _fecha_conversa(content: str) -> bool:
    ...
    t = (content or "").replace("﻿", "").strip().lower()
    if "[sticker]" in t or t.startswith("🏷️"):
        return True
    limpo = "".join(c for c in t if c.isalnum() or c.isspace()).replace("﻿", "").strip()
...
def conversa_precisa_resposta(anthropic_key: str, hist: list[dict]) -> bool:
    ...
    transcript = "\n".join(
        ...
        for m in hist[-6:] if (m.get("content") or "").replace("﻿", "").strip())
    if not transcript:
        return True
    try:
        import anthropic
        ...
        veredito = (msg.content[0].text or "").replace("﻿", "").strip().upper()
...
def main() -> int:
    ...
    zap = os.environ.get("UAZAPI_TOKEN_CEO", "").replace("﻿", "").strip()
    dry = os.environ.get("DRY_RUN", "") == "1"
    destino = os.environ.get("ALERTA_PARA", "5516992290338").replace("﻿", "").strip()
